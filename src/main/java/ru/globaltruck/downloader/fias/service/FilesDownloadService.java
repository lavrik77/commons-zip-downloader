...
import ru.globaltruck.downloader.fias.util.XmlFileParser;
...
import static ru.globaltruck.downloader.fias.util.FileNamePattern.ADDR_OBJ_PATTERN;
import static ru.globaltruck.downloader.fias.util.FileNamePattern.ADM_HIERARCHY_PATTERN;
import static ru.globaltruck.downloader.fias.util.FileNamePattern.HOUSE_PATTERN;
import static ru.globaltruck.downloader.fias.util.FileNamePattern.MUN_HIERARCHY_PATTERN;
        ...
        files.forEach(file -> {
            try {
                if (ADDR_OBJ_PATTERN.matcher(file.getFileName().toString()).find()) {
                    xmlFileParser.parse(file, handlerFactory.getAddressObjectHandler());
                }
                if (ADM_HIERARCHY_PATTERN.matcher(file.getFileName().toString()).find()) {
                    xmlFileParser.parse(file, handlerFactory.getAdmHierarchyHandler());
                }
                if (HOUSE_PATTERN.matcher(file.getFileName().toString()).find()) {
                    xmlFileParser.parse(file, handlerFactory.getHouseHandler());
                }
                if (MUN_HIERARCHY_PATTERN.matcher(file.getFileName().toString()).find()) {
                    xmlFileParser.parse(file, handlerFactory.getMunHierarchyHandler());
                }
            } catch (UnZipError e) {
